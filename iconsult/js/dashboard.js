...
function populateDashboardForm(profile) {
  ...
  const previewImg = document.getElementById('dashboard-avatar-preview');
  if (previewImg) {
    previewImg.src = profile.avatarUrl || `https://api.dicebear.com/7.x/bottts/svg?seed=${profile.username || 'user'}`;
  }
...
/**
 * Attach form submit and input listeners
 */
function setupEventListeners() {
  ...
  urlInput?.addEventListener('input', () => {
    ...
    if (avatarPreview && val) avatarPreview.src = val;
    currentProfile.avatarUrl = val;
    updateLivePreview(currentProfile);
  });

  const liveInputs = ['input-display-name', 'input-tagline', 'input-bio'];
