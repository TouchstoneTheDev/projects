...
export function createTrackerRow(report: CompanyAuditReportV11, outputDir?: string) {
  ...
  const email2 = contact2Obj && contact2Obj.address && !contact2Obj.address.includes('company.com') ? contact2Obj.address : 'Not Found';
  ...
  return {
    ...
    'Wave Score': report.totalViolations,
    'Axe Score': report.totalViolations,
    'LH Score': report.lighthouseAvgScore,
    'Screenshot link': githubMarkdownPath,
    'Contact Person 1': report.company.contactPerson && report.company.contactPerson !== 'N/A' ? report.company.contactPerson : `Compliance Lead (${report.company.companyName})`,
    'Designation 1': 'Head of Compliance & Digital Governance',
    'Email ID 1': email1,
    'Contact Person 2': person2,
    'Designation 2': 'Accessibility & Nodal Officer',
    'Email ID 2': email2
  };
}
...
export function exportDigitalV13TrackerFile(reports: CompanyAuditReportV11[], outputDir: string): void {
  ...
  const rows = reports.map(r => {
    ...
    const domain = r.resolution.resolvedUrl ? r.resolution.resolvedUrl.replace(/^https?:\/\/(www\.)?/, '').split('/')[0] : 'company.com';
    const githubScreenshotUrl = `${repoBaseUrl}/${runFolderName}/${safeName}/screenshots/${safeName}_Homepage_WAVE_Overlay.png`;
    const githubMarkdownPath = `![WAVE Tool Screenshot](${githubScreenshotUrl})`;
    ...
    return {
      ...
      'Wave Score': r.totalViolations,
      'Axe Score': r.totalViolations,
      'LH Score': r.lighthouseAvgScore,
      'Screenshot link': githubMarkdownPath,
      'Contact Person 1': r.company.contactPerson && r.company.contactPerson !== 'N/A' ? r.company.contactPerson : `Compliance Lead (${r.company.companyName})`,
      'Designation 1': 'Head of Compliance & Digital Governance',
      'Email ID 1': email1,
      'Contact Person 2': person2,
      'Designation 2': 'Accessibility & Nodal Officer',
  ...
  console.log(`\n📊 Generated v1.3 Semi-Final Digital Outreach Tracker: "${excelPath}"`);
}
