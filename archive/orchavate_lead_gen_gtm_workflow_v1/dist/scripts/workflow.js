...
const inferWebsiteFromEmail = async (emailId) => {
    if (!emailId)
        return undefined;
...
const searchWebsiteForCompany = async (companyName, emailId) => {
...
const hydrateCompanyRecord = async (company) => {
...
export const runWorkflow = async (trackerPath) => {
    ...
    for (const company of companies) {
        ...
        if (!hydratedCompany.websiteUrl) {
            ...
            resultRows.push(buildResultRow({
        ...
        await updateTracker(hydratedCompany.companyName, {
            status: scan.status,
        ...
        log.info({ module: 'workflow', company: hydratedCompany.companyName, status: scan.status }, 'Completed company scan');
    }
