...
const getCompanyNameFromRow = (row) => {
    const genericHeaderPattern = /^(name|registration no\.?|contact person|email[- ]?id|telephone|fax|city|state|pincode|from|to|country|exchange name|trade name|address|correspondence address|validity)$/i;
    ...
    const entries = Object.entries(row)
        ...
        if (!value)
            return false;
        if (genericHeaderPattern.test(key) || genericHeaderPattern.test(value))
            return false;
...
const normalizeRowToCompany = (row) => {
    ...
    const securityCode = getRowString(row, ['securityCode', 'SecurityCode', 'Security Code', 'Registration No.', 'registrationNo']) || undefined;
    const isin = getRowString(row, ['isin', 'ISIN']) || undefined;
    const websiteUrl = getRowString(row, ['websiteUrl', 'WebsiteUrl', 'Website', 'URL', 'url', 'website']) ||
        (Object.values(row)
            .filter((value) => typeof value === 'string')
